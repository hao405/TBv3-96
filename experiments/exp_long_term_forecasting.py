...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # 添加NSTS loss（如果模型支持）
                if hasattr(self.model, 'nsts_loss'):
                    nsts_weight = getattr(self.args, 'nsts_weight', 1e-6)
                    loss = loss + nsts_weight * self.model.nsts_loss
                elif hasattr(self.model, 'module') and hasattr(self.model.module, 'nsts_loss'):
                    # 处理DataParallel的情况
                    nsts_weight = getattr(self.args, 'nsts_weight', 1e-6)
                    loss = loss + nsts_weight * self.model.module.nsts_loss
    ...
    def test(self, setting, test=0):
        ...
        f = open("solar192to720_result_long_term_forecast.txt", 'a')
        f.write(setting + "  \n")
